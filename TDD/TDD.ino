...
#define WriteDelay 2
byte status=0;
unsigned long switchCombination = 0;
unsigned long oldSwitch = 0;
...
// Convert switchCombination to a note
int getNoteOn ( unsigned long switchCombination )
{
  int value = -1;
  ...
  if (value > -1)
  {
    value += startNote; // offset to note of C
  }
  return value;
}
...
void midiSend(byte stat, byte data1, byte data2) {    
  //  digitalWrite(LedPin,HIGH);  // indicate we're sending MIDI data
  delay(WriteDelay);
  Serial.print(stat);
  Serial.print(data1 + startNote);
  Serial.print(data2);
...
void loop ()
{
  if (switchCombination != oldSwitch)
  {
    ...
    midiSend ((0x80 | MIDIChannel), note, 0x00); 
  }
  oldSwitch = switchCombination;  
}
